...
- [ui-settings](../ui-settings/README.md) — the settings scope this plugin's tuner binds and the Plugins tab slot it fills.
...
The Host half is a class plugin: it installs the settings section and provides the `themeWallpaper` Remote namespace, which `packages/api/remotes` mounts for the application's Client assembly.
