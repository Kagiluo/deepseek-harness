...
- The settings namespace (`scheduler`) is the only user-authored layer, and `Config` is its composition base. Both describe the same task record through `taskSchema`; do not restate the fields in either layer.
